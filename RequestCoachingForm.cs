...
namespace assignment
{
    public class RequestCoachingForm : Form
    {
        ...
        {
            InitializeComponent();
            // populate lecturers from TrainerStore
            try
            {
                comboLecturers.Items.Clear();
                foreach (var t in TrainerStore.GetAll())
                {
                    var display = string.IsNullOrEmpty(t.Name) ? t.Username : t.Name + " (" + t.Username + ")";
                    comboLecturers.Items.Add(display);
                }
            }
            catch { }
        }
